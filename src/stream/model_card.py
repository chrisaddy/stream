...
def pr_curve_chart(precisions: list, recalls: list) -> go.Figure:
    ...
    fig.update_layout(
        **_LAYOUT, title="Precision-Recall Curve", xaxis_title="Recall", yaxis_title="Precision"
    )
    return fig
...
def feature_importance_chart(names: list[str], scores: list[float], top_k: int = 15) -> go.Figure:
    ...
    fig.update_layout(
        **_LAYOUT, title="Feature Importance", xaxis_title="Importance", yaxis_title=""
    )
    return fig
...
def calibration_chart(prob_true: list, prob_pred: list, class_label: str = "") -> go.Figure:
    ...
    fig.update_layout(
        **_LAYOUT,
        title=f"Calibration Curve {class_label}".strip(),
...
def confusion_matrix_chart(tp: int, fp: int, fn: int, tn: int) -> go.Figure:
    ...
    fig = go.Figure(
        go.Heatmap(
            ...
            text=text,
            texttemplate="%{text}",
            textfont=dict(size=14, color="#232136"),
            colorscale=[[0, "#393552"], [1, GREEN]],
            showscale=False,
        )
    )
...
def temporal_chart(timesteps: list[dict]) -> go.Figure:
    ...
    fig.update_layout(
        **_LAYOUT, title="Temporal Stability", xaxis_title="Timestep", yaxis_title="Score"
    )
    return fig
...
def regime_chart(regimes: dict) -> go.Figure:
    ...
    fig.update_layout(
        **_LAYOUT,
        title="MAE by Congestion Regime",
        xaxis_title="Regime",
        yaxis_title="MAE (sat/vB)",
    )
    return fig
